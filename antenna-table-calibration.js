...
const AntennaTableCalibration = (() => {
    ...
    function addPoint(headingDeg, measuredAzimuthDeg, slantRangeM, 
        ...
        const polarResult = polarCS_ShiftRotate(
            headingDeg, 0, measuredAzimuthDeg, slantRangeM, 0, 0
        );
        
        const latRad = Vincenty.deg2rad(antennaLatDeg);
        const lonRad = Vincenty.deg2rad(antennaLonDeg);
        const azmRad = Vincenty.deg2rad(polarResult.a_deg);
        
        const v = Vincenty.vincentyDirect(latRad, lonRad, azmRad, polarResult.r_a);
        const geoResult = v.converged ? v : Haversine.haversineDirect(latRad, lonRad, polarResult.r_a, azmRad);
        ...
        points.push({
            ...
            absAzimuthDeg: polarResult.a_deg,
            absRangeM: polarResult.r_a,
            antennaDepthM,
            beaconDepthM
        });
    }
    ...
    function computeCentroid() {
        ...
        return { lat: centroidLat, lon: centroidLon, count: validCount };
    }
    ...
    function buildTable(stepDeg = 1.0) {
        ...
        for (const pt of points) {
            ...
            errorPoints.push({
                encoderAngle, error, trueAzimuth: trueAzimuthDeg,
                expectedRelative: expectedRelativeBearing,
                measured: pt.measuredAzimuthDeg, heading: pt.headingDeg
            });
        }
        
        const numSectors = Math.ceil(360 / angleStep);
        const sectors = new Array(numSectors).fill(null).map((_, i) => ({
            errors: [],
            center: i * angleStep + angleStep / 2
        }));
        ...
        calibrationTable = {
            ...
            coverage: rawAngles.length / numSectors,
            centroidLat, centroidLon,
    ...
    function exportCSV() {
        ...
        return AntennaCorrector.formatToText(calibrationTable.encoderAngles, calibrationTable.errors);
    }
    ...
    function downloadCSV(filename) {
        ...
        AntennaCorrector.downloadCalibrationFile(
            calibrationTable.encoderAngles, calibrationTable.errors,
    ...
    function applyToManager() {
        ...
        AZMManager.loadAntennaCalibration(calibrationTable.encoderAngles, calibrationTable.errors);
        return true;
    }

    function getState() {
        if (isNaN(centroidLat) || isNaN(centroidLon)) computeCentroid();
        return {
    ...
    function exportFullData() {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            const deltas = GeoUtils.deltasByDegrees(pt.antennaLatDeg, pt.antennaLonDeg, centroidLat, centroidLon);
            let trueAzimuthDeg = Math.atan2(deltas.deltaLonM, deltas.deltaLatM) * 180 / Math.PI;
            if (trueAzimuthDeg < 0) trueAzimuthDeg += 360;
            ...
            lines.push([
                i, pt.antennaLatDeg.toFixed(8), pt.antennaLonDeg.toFixed(8),
                pt.headingDeg.toFixed(2), pt.measuredAzimuthDeg.toFixed(2),
                pt.slantRangeM.toFixed(2), pt.beaconLatDeg.toFixed(8),
                pt.beaconLonDeg.toFixed(8), trueAzimuthDeg.toFixed(4),
                expectedRelativeBearing.toFixed(4), error.toFixed(6)
            ].join(','));
        }
        ...
        for (let i = 0; i < calibrationTable.encoderAngles.length; i++) {
            lines.push([calibrationTable.encoderAngles[i].toFixed(3), calibrationTable.errors[i].toFixed(6)].join(','));
        }
    ...
    function downloadFullData(filename) {
        ...
        a.href = url;
        a.download = filename || 'antenna_calibration_full_' + new Date().toISOString().slice(0, 10) + '.csv';
    ...
    return {
        reset, addPoint, getCount, computeCentroid, buildTable,
        exportCSV, exportFullData, downloadCSV, downloadFullData,
        applyToManager, getState, setAngleStep,
